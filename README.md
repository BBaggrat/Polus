...
**Текущий статус: этап 3 — база и долгосрочная прогрессия.** Игрок возвращает добычу, улучшает убежище и снаряжение, собирает фрагменты карты и выбирает маршруты, которые меняют риск следующего выхода. Существующая PvP-механика сохранена как стычка в открытой топи.
...
4. Начать скрытое исследование или выйти в `OPEN_PVP`.
5. Читать живые записи, принимать решения и собирать добычу.
6. Вернуться на базу, чтобы сохранить ресурсы и продолжить прогрессию.
