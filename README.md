...
Sandalpunk / Полюс — компактная Telegram survival-RPG / Mini App о плаваниях по аномальному океану. Игрок держит лодку у старой пристани, выходит на Большую воду, принимает решения о риске, собирает детали, припасы и записи бортового журнала, встречает невозможные объекты, подводные угрозы и чужие лодки.
...
**Текущий статус: релизная сборка 1.0.** Игрок открывает бота, запускает Mini App, выходит на Большую воду через живой бортовой журнал, выбирает `HIDDEN` или `OPEN_PVP`, возвращает груз к причалу, улучшает лодку, модули и карту, затем повторяет цикл.
...
5. Начать тихое плавание или выйти в `OPEN_PVP`.
6. Читать живые записи, принимать решения, собирать груз и завершать цепочки.
7. Вернуться к причалу, чтобы сохранить ресурсы и продолжить модификации лодки.
